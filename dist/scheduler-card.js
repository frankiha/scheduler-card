(function (exports) {
    ...
    */
    ...
    function ExportToHass(userData, configData) {
        ...
        let action = lodash.pick(actionCfg, ['service', 'service_data']);
        lodash.assign(action, { entity: entityCfg.id });
    ...
    function CreateSlug(input) {
        let keys = lodash(input).keys();
        keys = keys.sort();
        let obj = {};
        lodash(keys).each(key => obj[key] = input[key]);
        return slugify(JSON.stringify(lodash.values(obj)).replace(/\W/g, ' '), '_');
    }
    ...
    function ImportFromHass(hassData, configData) {
        ...
        let actions = hassData.attributes['actions'].map(action => {
            ...
            let service_data = lodash.omit(action, ['service', 'entity']);
    ...
    function PrettyPrintDays(dayArray) {
        if (ComputeDaysType(dayArray) == 'daily')
            ...
            if (n)
                output_str = output_str.slice(0, n) + output_str.slice(n).replace(', ', ' and ');
    ...
    function PrettyPrintTime(timeData) {
        if (timeData.event) {
            ...
            if (Math.abs(offset) < 1 / 6)
                return `at ${timeData.event} (${timeData.time})`;
            else
                return `${res[2]}:${res[3]} ${res[1] == '+' ? 'after' : 'before'} ${timeData.event} (${timeData.time})`;
        }
        else {
    ...
    class Config {
        constructor(userConfig) {
            this.entities = {};
            this.groups = {};
            if (!userConfig['entities'] && !userConfig['domains'] && (userConfig.standardConfiguration === undefined || userConfig.standardConfiguration))
                this.userConfig = { domains: defaultDomainConfig };
            else
                this.userConfig = userConfig;
            if (userConfig.groups) {
                lodash.each(userConfig.groups, this.CreateGroup.bind(this));
            }
            this.discoverExisting = (userConfig.discoverExisting !== undefined) ? userConfig.discoverExisting : true;
        }
        CreateGroup(cfg, id) {
            let data = Object.assign({}, cfg);
            if (this.groups[id])
                return;
            lodash.defaults(data, {
        ...
        FindGroupForEntity(entity_id) {
            let domain = getDomainFromEntityId(entity_id);
            let res = lodash.find(this.groups, e => { return e.domains.includes(domain) || e.entities.includes(entity_id); });
            if (res)
                return true;
            else
                return false;
        ...
        AddEntityToGroup(entity_id) {
            ...
            if (this.groups[domain]) {
                ...
                this.CreateGroup({ entities: [entity_id], icon: getIconForDomain(domain) }, domain);
        }
        AddEntityInfo(entity_id, cfg) {
            if (this.entities[entity_id]) {
                Object.assign(this.entities[entity_id], lodash.omit(Object.assign({}, cfg), 'actions'));
                if (lodash(cfg).has('actions')) {
                    lodash(cfg.actions).each(action => {
                        let match = lodash(this.entities[entity_id]['actions']).find(e => { return CreateSlug(lodash.pick(e, ['service', 'service_data'])) == CreateSlug(lodash.pick(action, ['service', 'service_data'])); });
                        if (match)
                            return;
                ...
                let entry = Object.assign(Object.assign({}, cfg), { id: entity_id });
                lodash.defaults(entry, { actions: [] });
                this.entities[entity_id] = entry;
            }
            if (!this.FindGroupForEntity(entity_id))
                this.AddEntityToGroup(entity_id);
        }
        LoadEntities(entityList) {
            lodash.each(entityList, entity => {
            ...
            if (this.discoverExisting) {
                lodash(entityList)
                    ...
                    let service_data = lodash.omit(config, ['entity', 'service']);
                    if (service_data) {
                        ...
                        this.AddEntityInfo(config['entity'], {
                            actions: [lodash.pick(config, 'service')]
                        });
                    }
                });
            }
            lodash.each(this.entities, (cfg, entity_id) => {
                lodash.defaults(cfg, {
                    name: entityList[entity_id].attributes['friendly_name'],
                    icon: getIconForDomain(getDomainFromEntityId(entity_id))
                });
                Object.assign(cfg, {
                    actions: lodash.mapValues(cfg['actions'], action => {
                        let config = Object.assign({}, action);
                        lodash.defaults(config, {
                            name: action['service'],
                            icon: getIconForAction(action['service'])
                        });
                        Object.assign(config, { id: CreateSlug(lodash.pick(action, ['service', 'service_data'])) });
                        return config;
                    })
                });
            });
        ...
        GetGroups() {
            return lodash.mapValues(this.groups, el => {
                return lodash.pick(el, ['name', 'icon']);
            });
        }
        GetEntities(group_id) {
            ...
            if (!group_id)
                ...
                entities = lodash.pickBy(this.entities, (_entityCfg, entity_id) => {
                    let domain = getDomainFromEntityId(entity_id);
                    if (groupCfg['domains'] && groupCfg['domains'].includes(domain))
                        return true;
                    else if (groupCfg['entities'] && groupCfg['entities'].includes(entity_id))
                        return true;
                    else
                        return false;
                });
            }
            return lodash.mapValues(entities, el => {
                return lodash.pick(el, ['name', 'icon']);
            });
        }
        GetEntity(entity_id) {
        ...
        GetActions(entity_id) {
            let entityCfg = this.entities[entity_id];
            let actions = lodash.keyBy(entityCfg['actions'], 'id');
            return lodash.mapValues(actions, el => {
                return lodash.pick(el, ['name', 'icon']);
            });
        }
        GetAction(entity_id, action_id) {
            let entityCfg = this.entities[entity_id];
            return lodash.find(entityCfg.actions, { id: action_id });
        }
    }
...
`;
    ...
    function Type(item) {
        if (Array.isArray(item))
            ...
            return TYPE_OBJECT;
        else if (typeof item == typeof "x")
            return TYPE_STRING;
        else if (typeof item == typeof true)
            return TYPE_BOOLEAN;
        else if (typeof item == typeof 1)
            return TYPE_NUMBER;
        else
            return TYPE_OTHER;
    }
    ...
    function Required(item, key, type) {
        if (!item || !item.hasOwnProperty(key))
            return false;
        else
            return IsType(item[key], type);
    }
    function Optional(item, key, type) {
        if (!item || !item.hasOwnProperty(key))
            return true;
        else
            return IsType(item[key], type);
    ...
    function ValidateConfig(config) {
        if (config['entities'] !== undefined) {
            try {
                if (!Required(config, 'entities', TYPE_OBJECT))
                    throw `Configuration for 'entities' should be of type '${TYPE_OBJECT}', but got '${Type(config['entities'])}'.`;
                lodash.each(config['entities'], (entityEntry, entry) => {
                    if (!IsType(entityEntry, TYPE_OBJECT))
                        throw `In '${entry}: (...)': expected type '${TYPE_OBJECT}', but got '${Type(entityEntry)}'.`;
                    if (!Optional(entityEntry, 'name', TYPE_STRING))
                        throw `In ${entry} at 'name: ${entityEntry['name']}': expected type '${TYPE_STRING}', but got '${Type(entityEntry['name'])}'.`;
                    if (!Optional(entityEntry, 'icon', TYPE_STRING))
                        throw `In ${entry} at 'icon: ${entityEntry['icon']}': expected type '${TYPE_STRING}', but got '${Type(entityEntry['icon'])}'.`;
                    if (!Optional(entityEntry, 'actions', TYPE_ARRAY))
                        throw `In ${entry} at 'actions: (...)': expected type '${TYPE_ARRAY}', but got '${Type(entityEntry['actions'])}'.`;
                    if (entityEntry) {
                        lodash.each(entityEntry['actions'], actionCfg => {
                            if (!Required(actionCfg, 'service', TYPE_STRING))
                                throw `In ${entry}->actions at 'service: ${actionCfg['service']}': expected type '${TYPE_STRING}', but got '${Type(actionCfg['service'])}'.`;
                            if (!Optional(actionCfg, 'name', TYPE_STRING))
                                throw `In ${entry}->actions at 'name: ${actionCfg['name']}': expected type '${TYPE_STRING}', but got '${Type(actionCfg['name'])}'.`;
                            if (!Optional(actionCfg, 'icon', TYPE_STRING))
                                throw `In ${entry}->actions at 'icon: ${actionCfg['icon']}': expected type '${TYPE_STRING}', but got '${Type(actionCfg['icon'])}'.`;
                            if (!Optional(actionCfg, 'service_data', TYPE_OBJECT))
                                throw `In ${entry}->actions at 'service_data: ${actionCfg['service_data']}': expected type '${TYPE_OBJECT}', but got '${Type(actionCfg['service_data'])}'.`;
                        });
                    }
                });
            }
            catch (e) {
                throw new Error(`Invalid configuration provided for 'entities'. ${e}`);
            }
        }
        if (config['domains'] !== undefined) {
            try {
                if (!Required(config, 'domains', TYPE_OBJECT))
                    throw `Configuration for 'domains' should be of type '${TYPE_OBJECT}', but got '${Type(config['domains'])}'.`;
                lodash.each(config['domains'], (domainEntry, entry) => {
                    if (!IsType(domainEntry, TYPE_OBJECT))
                        throw `In '${entry}: (...)': expected type '${TYPE_OBJECT}', but got '${Type(domainEntry)}'.`;
                    if (!Optional(domainEntry, 'name', TYPE_STRING))
                        throw `In ${entry} at 'name: ${domainEntry['name']}': expected type '${TYPE_STRING}', but got '${Type(domainEntry['name'])}'.`;
                    if (!Optional(domainEntry, 'icon', TYPE_STRING))
                        throw `In ${entry} at 'icon: ${domainEntry['icon']}': expected type '${TYPE_STRING}', but got '${Type(domainEntry['icon'])}'.`;
                    if (!Optional(domainEntry, 'actions', TYPE_ARRAY))
                        throw `In ${entry} at 'actions: (...)': expected type '${TYPE_ARRAY}', but got '${Type(domainEntry['actions'])}'.`;
                    if (domainEntry) {
                        lodash.each(domainEntry['actions'], actionCfg => {
                            if (!Required(actionCfg, 'service', TYPE_STRING))
                                throw `In ${entry}->actions at 'service: ${actionCfg['service']}': expected type '${TYPE_STRING}', but got '${Type(actionCfg['service'])}'.`;
                            if (!Optional(actionCfg, 'icon', TYPE_STRING))
                                throw `In ${entry}->actions at 'icon: ${actionCfg['icon']}': expected type '${TYPE_STRING}', but got '${Type(actionCfg['icon'])}'.`;
                ...
                throw new Error(`Invalid configuration provided for 'domains'. ${e}`);
            }
        }
        if (config['groups'] !== undefined) {
            try {
                if (!Required(config, 'groups', TYPE_OBJECT))
                    throw `Configuration for 'domains' should be of type '${TYPE_OBJECT}', but got '${Type(config['groups'])}'.`;
                lodash.each(config['groups'], (groupEntry, entry) => {
                    if (!IsType(groupEntry, TYPE_OBJECT))
                        throw `In '${entry}: (...)': expected type '${TYPE_OBJECT}', but got '${Type(groupEntry)}'.`;
                    if (!Optional(groupEntry, 'name', TYPE_STRING))
                        throw `In ${entry} at 'name: ${groupEntry['name']}': expected type '${TYPE_STRING}', but got '${Type(groupEntry['name'])}'.`;
                    if (!Optional(groupEntry, 'icon', TYPE_STRING))
                        throw `In ${entry} at 'icon: ${groupEntry['icon']}': expected type '${TYPE_STRING}', but got '${Type(groupEntry['icon'])}'.`;
                    if (!Optional(groupEntry, 'entities', TYPE_ARRAY))
                        throw `In ${entry} at 'icon: ${groupEntry['entities']}': expected type '${TYPE_ARRAY}', but got '${Type(groupEntry['entities'])}'.`;
                    if (!Optional(groupEntry, 'domains', TYPE_ARRAY))
                        throw `In ${entry} at 'icon: ${groupEntry['domains']}': expected type '${TYPE_ARRAY}', but got '${Type(groupEntry['domains'])}'.`;
                    if (groupEntry) {
                        lodash.each(groupEntry['entities'], entityCfg => {
                            if (!IsType(entityCfg, TYPE_STRING))
                                throw `In ${entry}->entities at '${entityCfg}': expected type '${TYPE_STRING}', but got '${Type(entityCfg)}'.`;
                        });
                        lodash.each(groupEntry['domains'], domainCfg => {
                            if (!IsType(domainCfg, TYPE_STRING))
                                throw `In ${entry}->domains at '${domainCfg}': expected type '${TYPE_STRING}', but got '${Type(domainCfg)}'.`;
                        });
                    }
                });
            }
            catch (e) {
                throw new Error(`Invalid configuration provided for 'groups'. ${e}`);
            }
        }
        if (!Optional(config, 'discoverExisting', TYPE_BOOLEAN))
            throw new Error(`Invalid configuration provided for 'discoverExisting': expected type '${TYPE_BOOLEAN}', but got '${Type(config['discoverExisting'])}'.`);
        if (!Optional(config, 'standardConfiguration', TYPE_BOOLEAN))
            throw new Error(`Invalid configuration provided for 'standardConfiguration': expected type '${TYPE_BOOLEAN}', but got '${Type(config['standardConfiguration'])}'.`);
    }
    ...
    exports.SchedulerCard = class SchedulerCard extends LitElement {
        ...
        update_entries(hass) {
            let entries = lodash.filter(hass.states, entity => IsSchedulerEntity(entity.entity_id))
        ...
        _handleTouchStart(e) {
            let thumbElement;
            thumbElement = e.target;
            if (!thumbElement)
                return;
            let parentElement = thumbElement.parentNode;
            let trackElement = parentElement.parentElement;
            if (!trackElement)
                return;
            let trackCoords = trackElement.getBoundingClientRect();
            let leftNeighbour = parentElement.previousElementSibling;
            let rightNeighbour = parentElement.nextElementSibling;
            ...
            let segments = Array.from(trackElement.getElementsByClassName("slider-segment"));
            ...
            let mouseMoveHandler = function (e) {
                ...
                if (x < 0)
                    x = 0;
                else if (x > trackCoords.width)
                    x = trackCoords.width;
                if (x > (availableWidth + xStart))
                    x = availableWidth + xStart;
                if (x < xStart)
                    x = xStart;
        ...
        _handleSegmentClick(e) {
            ...
            let trackElement = segment.parentElement;
            if (!trackElement)
                return;
            let segments = Array.from(trackElement.getElementsByClassName("slider-segment"));
        ...
        _sliderAdd() {
            let segments = Array.from(this.shadowRoot.querySelectorAll(".slider-segment"));
            let active_segment = -1;
            segments.forEach((el, i) => {
                if (el.classList.contains("active"))
                    active_segment = i;
            });
            if (segments.length >= 10 || active_segment == -1)
                return;
        ...
        _sliderRemove() {
            ...
            if (segments.length == 2 || active_segment == -1)
                return;
            let cutIndex = active_segment;
            if (cutIndex == (segments.length - 1))
                cutIndex--;
      ...
      `);
        ...
        _updateMarker(e) {
            ...
            if (detail.hasOwnProperty('index')) {
                ...
                if (index >= this.sliderSettings.stops.length)
                    return;
      ...
      `;
        ...
        newItem() {
            this.selection = lodash.extend(Object.assign({}, DefaultUserSelection), {
                newItem: true,
            });
            this.requestUpdate();
        }
    ...
    `];
      ...
      <div class="list-item${entry['enabled'] ? '' : ' disabled'}" @click="${() => this.editItem(entry.id)}">
        ...
        <div class="list-item-time">
          ${PrettyPrintTime(lodash.pick(entry.entries[0], ['time', 'event', 'offset']))}
        </div>
      ...
      `;
        ...
        editItem(entity_id) {
            let data = lodash.find(this.entries, { id: entity_id });
            this.selection = lodash.extend(Object.assign({}, DefaultUserSelection), {
                editItem: entity_id,
        ...
        getGroups() {
            ...
            if (!lodash(groups).size())
                return [html `<div class="text-field">There are no groups defined</div>`];
            return lodash.map(groups, (el, key) => {
                return html `
      ...
      `;
        ...
        getEntities() {
            if (!this.selection.group)
                return [html `<div class="text-field">Select a group first</div>`];
            let entities = this.Config.GetEntities(this.selection.group);
            if (!lodash(entities).size())
                return [html `<div class="text-field">There are no entities in this group</div>`];
            return lodash.map(entities, (el, key) => {
                return html `
      ...
      `;
        ...
        getActions() {
            if (!this.selection.entity)
                return [html `<div class="text-field">Select an entity first</div>`];
            let actions = this.Config.GetActions(this.selection.entity);
            if (!lodash(actions).size())
                return [html `<div class="text-field">There are no actions for this entity</div>`];
            return lodash.map(actions, (el, key) => {
                return html `
        ...
        updateDays(action) {
            ...
            if (daysTypes.includes(action))
                ...
                if (!this.selection.days.includes(Number(action)))
                    ...
                    lodash.pull(this.selection.days, Number(action));
    ...
    ], exports.SchedulerCard.prototype, "_handleTouchStart", null);
